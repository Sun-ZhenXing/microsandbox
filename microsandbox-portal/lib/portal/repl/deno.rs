//! Deno engine implementation for code execution in a sandboxed environment.
//!
//! This module provides a Deno-based code execution engine that:
//! - Runs TypeScript/JavaScript code in a Deno subprocess
//! - Captures and streams stdout/stderr output
//! - Manages process lifecycle and cleanup
//! - Provides non-blocking evaluation of TypeScript code
//!
//! The engine uses Deno's REPL mode with customized settings to
//! disable prompts and ensure clean output handling.
...
impl Engine for DenoEngine {
    async fn initialize(&mut self) -> Result<(), EngineError> {
        ...
        // Start the Deno process manager in a separate task
        tokio::spawn(async move {
            // Start Deno process in REPL mode
            let mut process = match Command::new("deno")
                .args(&["repl", "--no-prompt", "--unstable"])
                ...
                .spawn()
            {
                Ok(p) => p,
                Err(e) => {
                    eprintln!("Failed to start Deno process: {}", e);
                    return;
                }
            };
            ...
            let mut stdin = match process.stdin.take() {
                Some(s) => s,
                None => {
                    eprintln!("Failed to open Deno stdin");
                    return;
                }
            };
            ...
            let stdout = match process.stdout.take() {
                Some(s) => s,
                None => {
                    eprintln!("Failed to open Deno stdout");
                    return;
                }
            };
            ...
            let stderr = match process.stderr.take() {
                Some(s) => s,
                None => {
                    eprintln!("Failed to open Deno stderr");
                    return;
                }
            };
            ...
            tokio::task::spawn_blocking(move || {
                ...
                loop {
                    ...
                    match line_result {
                        Ok(Some(line)) => {
                            // Skip Deno REPL response tags and prompts
                            if !line.trim().is_empty()
                                && !line.starts_with("> ")
                                && !line.starts_with("Deno ")
                                && !line.contains("exit using ctrl+d")
                            {
                ...
                tokio::select! {
                    ...
                    Some(eval_req) = eval_rx.recv() => {
                        ...
                        let result = async {
                            ...
                            // Write code to Deno process
                            stdin.write_all(code_with_marker.as_bytes()).await.map_err(|e| {
                                EngineError::Evaluation(format!("Failed to send code to Deno: {}", e))
                            })?;

                            // Flush to ensure code is processed
                            stdin.flush().await.map_err(|e| {
                                EngineError::Evaluation(format!("Failed to flush code to Deno: {}", e))
                            })?;
                    ...
                    _ = stdout_done_rx.recv() => {
                        eprintln!("Deno stdout handler exited");
                        break;
                    }
                    _ = stderr_done_rx.recv() => {
                        eprintln!("Deno stderr handler exited");
                        break;
                    }
                }
            }
    ...
    async fn eval(
        ...
        let eval_tx = self.eval_tx.as_ref().ok_or_else(|| {
            EngineError::Unavailable("Deno engine not initialized".to_string())
        })?;
        ...
        eval_tx
            ...
            .await
            .map_err(|_| EngineError::Unavailable("Deno process channel closed".to_string()))?;

        // Wait for completion
        done_rx
            .await
            .map_err(|_| EngineError::Unavailable("Deno evaluation cancelled".to_string()))?
    }
...
/// Create a new Deno engine instance
pub fn create_engine() -> Result<Box<dyn Engine>, EngineError> {
    Ok(Box::new(DenoEngine::new()))
}
